    ...
    for await (var key of client.scanIterator({ MATCH: 'order:*', COUNT: 100 })) {
      ...
      try {
        ...
        records.push({
          ...
          orderType: o.orderType || 'single',
          items: (o.items||[]).map(function(it){ return { qty: it.qty, unitPrice: it.unitPrice }; }),
          redeemedFreeCup: !!o.redeemedFreeCup,
          subtotal: o.subtotal, discount: o.discount, total: o.total,
          status: o.status,
          subDays: o.subDays || null
        });
      } catch (e) {}
    }
